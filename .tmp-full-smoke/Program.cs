...
Console.WriteLine("inspector.window=" + inspector.Execute("focus_window"));
...
try
{
    var ask = await runtime.AskAsync("openai", "", "companion", false, "Reply with exactly READY and nothing else.", false, new List<ConversationTurn>(), Array.Empty<KnowledgeChunk>());
    Console.WriteLine("ask.openai=ok");
    Console.WriteLine("ask.openai.response=" + ask.CleanResponseText.Replace(Environment.NewLine, " "));
}
catch (Exception ex)
{
    Console.WriteLine("ask.openai=fail");
    Console.WriteLine(ex.GetType().Name + ":" + ex.Message);
}

try
{
    var ask = await runtime.AskAsync("anthropic", "", "companion", false, "Reply with exactly READY and nothing else.", false, new List<ConversationTurn>(), Array.Empty<KnowledgeChunk>());
    Console.WriteLine("ask.anthropic=ok");
    Console.WriteLine("ask.anthropic.response=" + ask.CleanResponseText.Replace(Environment.NewLine, " "));
}
catch (Exception ex)
{
    Console.WriteLine("ask.anthropic=fail");
    Console.WriteLine(ex.GetType().Name + ":" + ex.Message);
}
...
try
{
    ...
    Console.WriteLine("elevenlabs.tts=ok");
    Console.WriteLine(ttsPath);
}
catch (Exception ex)
{
    Console.WriteLine("elevenlabs.tts=fail");
    Console.WriteLine(ex.GetType().Name + ":" + ex.Message);
}

try
{
    var wavPath = Path.Combine(@"C:\tmp\clippy_rw\.tmp-full-smoke", "elevenlabs-input.wav");
    var voice = Activator.CreateInstance(Type.GetTypeFromProgID("SAPI.SpVoice")!);
    var stream = Activator.CreateInstance(Type.GetTypeFromProgID("SAPI.SpFileStream")!);
    stream!.GetType().InvokeMember("Open", System.Reflection.BindingFlags.InvokeMethod, null, stream, new object[] { wavPath, 3, false });
    voice!.GetType().InvokeMember("AudioOutputStream", System.Reflection.BindingFlags.SetProperty, null, voice, new[] { stream });
    voice.GetType().InvokeMember("Speak", System.Reflection.BindingFlags.InvokeMethod, null, voice, new object[] { "Hallo Zippy Test eins zwei drei" });
    stream.GetType().InvokeMember("Close", System.Reflection.BindingFlags.InvokeMethod, null, stream, null);
    var transcript = await runtime.TranscribeAudioFileAsync(wavPath);
    Console.WriteLine("elevenlabs.stt=ok");
    Console.WriteLine(transcript);
}
catch (Exception ex)
{
    Console.WriteLine("elevenlabs.stt=fail");
    Console.WriteLine(ex.GetType().Name + ":" + ex.Message);
}
